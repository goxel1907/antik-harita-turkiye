...
- If OpenCode Free rejects 9Router REST with its provider-side `free tier can only be used from within OpenCode` restriction, BrainHub may use `openrouter/free` for Vision only when the user explicitly configures their own OpenRouter API key. This remains a zero-price/free-model fallback and does not authorize Kiro or any paid route. Vision success still requires actual 9-image model verification; provider metadata alone is insufficient.
