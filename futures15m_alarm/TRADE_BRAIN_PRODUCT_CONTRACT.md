...
- If OpenCode Free rejects 9Router REST with its provider-side `free tier can only be used from within OpenCode` restriction, BrainHub must reuse the OpenRouter provider already configured in 9Router instead of requesting or storing a second OpenRouter key. Only 9Router-discovered `openrouter/*:free` candidates (plus the zero-price `openrouter/openrouter/free` router) are eligible for this Vision fallback. This does not authorize Kiro or any paid route. Vision success still requires actual 9-image model verification; provider metadata alone is insufficient.
