...
- Every model-generated Leader 9TF plan, targeted or advisory, must also return explicit `SUPPORT_TFS`, `VETO_TFS`, and `FORMING_CONTEXT`. For each timeframe 1m/3m/5m/15m/30m/45m/1h/4h/1D it must provide a Turkish summary plus separate WHY, exact WAIT condition, SUPPORT/VETO/NEUTRAL role, forming-candle context, and timeframe-specific risk. Per-timeframe `TF_*_ROLE` fields are canonical for the displayed support/veto set because they are the granular model judgment. The model's aggregate `SUPPORT_TFS`/`VETO_TFS` lines are retained as declared summaries; a disagreement is surfaced as a model-consistency warning and never silently presented as agreement. Missing/invalid per-timeframe roles or other mandatory fields remain `REVIEW_REQUIRED`.
- Android/diagnostics must render model interpretation and deterministic evidence as separate labeled sections. Deterministic RSI/BOS/FVG/pattern/SMC summaries may corroborate or contradict the model, but must never be presented as if the model visually read them.
