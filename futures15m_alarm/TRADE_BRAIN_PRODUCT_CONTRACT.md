...
- Every model-generated Leader 9TF plan, targeted or advisory, must also return explicit `SUPPORT_TFS`, `VETO_TFS`, and `FORMING_CONTEXT`. For each timeframe 1m/3m/5m/15m/30m/45m/1h/4h/1D it must provide a Turkish summary plus separate WHY, exact WAIT condition, SUPPORT/VETO/NEUTRAL role, forming-candle context, and timeframe-specific risk. The declared support/veto lists must agree with per-timeframe roles; incomplete or contradictory model output is `REVIEW_REQUIRED`.
