...
- If OpenCode Free rejects 9Router REST with its provider-side `free tier can only be used from within OpenCode` restriction, BrainHub may retry only explicitly allowlisted free models through OpenCode's documented free inference endpoint. This remains a free-model transport fallback, not permission to switch to a paid model or imitate a restricted client. Vision success still requires actual 9-image model verification.
