...
class BinanceLiveTransport {
  ...
  async reducePosition({ symbol, side, quantity = null, fraction = 1, credentials } = {}) {
    const sym=String(symbol||'').trim().toUpperCase();
    const dir=String(side||'').trim().toUpperCase();
    const frac=Math.max(0,Math.min(1,finite(fraction)??1));
    if(!/^[A-Z0-9]{1,28}USDT$/.test(sym))return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['SYMBOL_INVALID']};
    if(!['LONG','SHORT'].includes(dir))return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['SIDE_INVALID']};
    if(!(frac>0))return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['REDUCE_FRACTION_INVALID']};
    const apiKey=text(credentials?.apiKey),apiSecret=text(credentials?.apiSecret);
    if(!apiKey||!apiSecret)return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['BINANCE_CREDENTIALS_REQUIRED']};
    try{
      await this._syncServerTime();
      const [exchangeInfo,mode,risk] = await Promise.all([
        this._fetchJson('GET','/fapi/v1/exchangeInfo'),
        this._fetchJson('GET','/fapi/v1/positionSide/dual',{credentials,signed:true}),
        this._fetchJson('GET','/fapi/v3/positionRisk',{params:{symbol:sym},credentials,signed:true})
      ]);
      const symbolInfo=Array.isArray(exchangeInfo?.symbols)?exchangeInfo.symbols.find(x=>String(x?.symbol||'').toUpperCase()===sym):null;
      if(!symbolInfo||symbolInfo.status!=='TRADING'||symbolInfo.contractType!=='PERPETUAL'||symbolInfo.quoteAsset!=='USDT'){
        return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['SYMBOL_NOT_REDUCE_ELIGIBLE']};
      }
      const hedgeMode=mode?.dualSidePosition===true;
      const rows=Array.isArray(risk)?risk.filter(x=>String(x?.symbol||'').toUpperCase()===sym):[];
      const row=rows.find(x=>{
        const amt=finite(x?.positionAmt)||0;
        if(Math.abs(amt)<=0)return false;
        if(hedgeMode)return String(x?.positionSide||'').toUpperCase()===dir;
        return dir==='LONG'?amt>0:amt<0;
      })||null;
      const currentQty=Math.abs(finite(row?.positionAmt)||0);
      if(!(currentQty>0))return {ok:true,orderPlaced:false,execution:'POSITION_ALREADY_FLAT',symbol:sym,side:dir,remainingQty:0,reasons:[]};
      const lot=filterOf(symbolInfo,'MARKET_LOT_SIZE')||filterOf(symbolInfo,'LOT_SIZE');
      const step=finite(lot?.stepSize),minQty=finite(lot?.minQty);
      if(step===null||step<=0||minQty===null)return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['MARKET_LOT_FILTER_REQUIRED']};
      const cap=finite(quantity);
      const baseQty=cap!==null&&cap>0?Math.min(currentQty,cap):currentQty;
      let reduceQty=frac>=0.999999?floorToStep(currentQty,step):floorToStep(baseQty*frac,step);
      if(reduceQty===null||reduceQty<minQty||reduceQty<=0)return {ok:false,orderPlaced:false,execution:'POSITION_REDUCE_BLOCKED',reasons:['REDUCE_QTY_BELOW_MIN']};
      if(reduceQty>currentQty)reduceQty=floorToStep(currentQty,step);
      const params={
        symbol:sym,
        side:dir==='LONG'?'SELL':'BUY',
        positionSide:hedgeMode?dir:'BOTH',
        type:'MARKET',
        quantity:decimal(reduceQty),
        newClientOrderId:'JEVR'+crypto.createHash('sha256').update(sym+'|'+dir+'|'+this.clock()+'|'+reduceQty).digest('hex').slice(0,28),
      ...
      if(!(ack?.orderId!==null&&ack?.orderId!==undefined)||!(executedQty>0)){
        return {ok:false,orderPlaced:true,execution:'POSITION_REDUCE_REVIEW_REQUIRED',symbol:sym,side:dir,requestedQty:reduceQty,orderId:ack?.orderId??null,executedQty,manualReviewRequired:true,reasons:['REDUCE_FILL_STATE_UNCLEAR']};
      }
      ...
      try{
        ...
        const afterRows=Array.isArray(after)?after.filter(x=>String(x?.symbol||'').toUpperCase()===sym):[];
        const afterRow=afterRows.find(x=>{
          const amt=finite(x?.positionAmt)||0;
          if(Math.abs(amt)<=0)return false;
          if(hedgeMode)return String(x?.positionSide||'').toUpperCase()===dir;
          return dir==='LONG'?amt>0:amt<0;
        })||null;
        remainingQty=Math.abs(finite(afterRow?.positionAmt)||0);
      }catch{}
      const fullRequested=frac>=0.999999;
      const closeVerified=remainingQty!==null ? (fullRequested?remainingQty<Math.max(step/2,1e-12):remainingQty<currentQty) : false;
      return {
        ok:true,orderPlaced:true,liveAllowed:true,
        execution:fullRequested?'JEV_EXIT_REDUCE_ONLY_MARKET':'JEV_PARTIAL_REDUCE_ONLY_MARKET',
        symbol:sym,side:dir,requestedQty:reduceQty,executedQty,orderId:ack?.orderId??null,
        remainingQty,closeVerified,fullRequested,reduceOnly:!hedgeMode,positionSide:params.positionSide,reasons:[]
      };
    }catch(e){
      return {ok:false,orderPlaced:false,liveAllowed:false,execution:'POSITION_REDUCE_FAILED',
        symbol:sym,side:dir,transport:{attempted:true,requestSent:Boolean(e?.requestSent)},
        exchangeError:e?.body||null,reasons:[String(e?.message||'POSITION_REDUCE_FAILED').slice(0,160)]};
    }
  }
