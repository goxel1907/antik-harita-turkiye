...
function kiroFreeQuotaVisionPool(ccfg,role='STRUCTURE',includeCooldown=false){
  if(ccfg?.allowKiroFreeQuotaVision!==true)return [];
  const kiro=uniqueModels(cfg.kiro||[]);
  const configured=uniqueModels(Array.isArray(ccfg?.kiroFreeQuotaVisionModels)
    ? ccfg.kiroFreeQuotaVisionModels
    : ['kr/claude-sonnet-4.5','kr/claude-haiku-4.5']).filter(x=>kiro.includes(x));
  const available=includeCooldown
    ? configured
    : configured.filter(x=>!visionBlocked(x));
  return rankPool(available,role,true);
}
async function runKiroFreeQuotaVisionFallback(body,localError){
  const j=body&&typeof body==='object'?body:{};
  let ccfg={};try{ccfg=readCommitteeConfig();}catch{}
  const role=normalizeRole(j.role||'STRUCTURE');
  const models=kiroFreeQuotaVisionPool(ccfg,role,false);
  if(!models.length){
    const err=localError instanceof Error?localError:new Error(String(localError||'LOCAL_VISION_FAILED'));
    err.freeQuotaFallback={attempted:false,reason:'NO_KIRO_FREE_QUOTA_VISION_ROUTE'};
    throw err;
  }
  const vision=multimodalUserContent(j.prompt,j.images);
  if(!vision.images.length)throw localError;
  const system=[roleInstruction(role),String(j.system||'').trim(),
    'LOCAL_VISION_FREE_QUOTA_FAILOVER: local Vision failed. This is an explicitly enabled connected-account free-quota fallback only. Preserve the exact requested output schema. Advisory only; do not place orders.'
  ].filter(Boolean).join(' ');
  const messages=[];
  if(system)messages.push({role:'system',content:system});
  messages.push({role:'user',content:vision.content});
  const timeoutMs=Math.max(30000,Math.min(180000,Number(ccfg?.visionTimeoutMs||90000)));
  const failures=[];
  const attempted=[];
  for(const model of models){
    ...
    try{
      const out=await callModel(model,messages,timeoutMs,{temperature:0,maxTokens:4096});
      const durationMs=Date.now()-started;
      visionState.set(model,{ok:true,at:Date.now(),error:null,durationMs});
      log('VISION FREE QUOTA FAILOVER OK model='+model+' localError='+String(localError?.message||localError||'').slice(0,120));
      return {
        ok:true,role,mode:'kiro_free_quota_fallback',degraded:true,
        degradedReason:'LOCAL_VISION_FAILED_FREE_QUOTA_FALLBACK',
        requiredAnalystReplies:1,requiredVisionAnalystReplies:1,receivedAnalystReplies:1,
        forceVisionProbe:false,visionTimeoutMs:timeoutMs,visionParallelAnalysts:1,
        localVisionTwoStage:false,localVisionBatchSize:null,
        visionKiroFreeQuota:true,localVisionFailed:true,
        localVisionError:String(localError?.message||localError||'').slice(0,500),
        disagreement:false,verdictConsensus:null,
        vision:{attached:vision.images.length,timeframes:vision.images.map(x=>x.tf),modes:vision.images.map(x=>x.mode)},
        analysts:[{ok:true,model,text:out.text,durationMs,freeQuotaFallback:true}],
        failed:failures,attemptedModels:attempted,judge:{used:false},
        model,text:out.text
      };
    }catch(e){
      const durationMs=Date.now()-started;
      ...
      failures.push({model,error:msg,durationMs});
      log('VISION FREE QUOTA FAILOVER FAIL model='+model+' '+msg.slice(0,160));
    }
  }
  const err=new Error('committee unavailable: local vision failed and Kiro free-quota fallback failed');
  err.committee={
    ok:false,error:'no vision analyst replies',role,required:1,received:0,
    vision:{attached:vision.images.length,timeframes:vision.images.map(x=>x.tf),modes:vision.images.map(x=>x.mode)},
    attemptedModels:attempted,localVisionError:String(localError?.message||localError||'').slice(0,500),
    kiroFreeQuotaFallback:true,failures
  };
  throw err;
}
...
function compactLocalFinalizeContext(localContext){
  ...
  // R2536: Vision may legitimately choose the opposite side from the scanner attention hint.
  // Provide deterministic level candidates for BOTH sides so schema repair never fails only because
  // sourceCandidate.side differs from the final visual hypothesis.
  const triggerCandidates=typeof pipeline?.triggerCandidatesForPlan==='function'
    ? ['LONG','SHORT'].flatMap(side=>pipeline.triggerCandidatesForPlan({frames:c.frames||{}},side).map(x=>({...x,side})))
    : [];
  return {
...
async function runLocalVisionCommitteeUnlocked(body){
  ...
  try{
    ...
    if(forceVisionProbe){
      ...
      // R2536: the sovereign JEV path requests Vision as EVIDENCE_ONLY. It does not need
      // legacy global plan fields such as TRIGGER_LEVEL_ID / INVALIDATION_LEVEL_ID.
      // Returning the extracted TF evidence directly prevents a harmless schema omission
      // from becoming DETAIL_RUN_ERROR while preserving numeric authority and JEV final authority.
      if(j.evidenceOnly===true){
        const durationMs=Date.now()-started;
        ...
        text=[
          ...
          'VISION_AUTHORITY: EVIDENCE_ONLY',
          'EXECUTION: ADVISORY_ONLY'
        ].filter(Boolean).join('\n');
        visionState.set(model,{ok:true,at:Date.now(),error:null,durationMs});
        setLocalVisionProgress('DETAIL_RUN_COMPLETE',{runId,model,lastStageDurationMs:durationMs,error:null});
        const analyst={
          ok:true,model,text,durationMs,
          localVisionTwoStage:true,localVisionBatchSize:3,localVisionSingleTfFallback:true,
          visionExtractionText:visionText.slice(0,5000),visionDurationMs,batchDurations,
          globalFinalizeSplit:false,evidenceOnly:true
        };
        ...
        log('COMMITTEE LOCAL_DIRECT EVIDENCE_ONLY OK role='+role+' model='+model+' visionCharts='+vision.images.length);
        return {
          ok:true,role,mode:'degraded_single',degraded:minReplies>1,
          degradedReason:minReplies>1?'DEGRADED_1_ANALYST':null,
          requiredAnalystReplies:minReplies,requiredVisionAnalystReplies:minVisionReplies,
          receivedAnalystReplies:1,forceVisionProbe:false,visionTimeoutMs:local.timeoutMs,
          visionParallelAnalysts:1,localVisionTwoStage:true,localVisionBatchSize:3,
          localVisionSingleTfFallback:true,visionKiroFreeQuota:false,disagreement:false,
          verdictConsensus:null,evidenceOnly:true,
          vision:{attached:vision.images.length,timeframes:vision.images.map(x=>x.tf),modes:vision.images.map(x=>x.mode)},
          analysts:[analyst],failed:[],judge:{used:false},model,text
        };
      }
      ...
      // R2536: model text repairs can omit only the level IDs even when deterministic closed-candle
      // candidates already exist. Fill ONLY missing TRIGGER_TF / TRIGGER_LEVEL_ID /
      // INVALIDATION_LEVEL_ID from same-TF deterministic candidates; never change SIDE/STATUS.
      let deterministicCoreRepairMeta=null;
      if(!coreContract.ok&&coreContract.missing.every(x=>['TRIGGER_TF','TRIGGER_LEVEL_ID','INVALIDATION_LEVEL_ID'].includes(x))){
        deterministicCoreRepairMeta=deterministicCoreLevelRepair(coreContract,finalizeContext);
        if(deterministicCoreRepairMeta?.applied){
          coreContract=globalCoreContract(normalizedLabeledText(deterministicCoreRepairMeta.contract,coreLabels,80));
        }
      }
      ...
      analystMeta={
        ...
        deterministicCoreLevelRepair:deterministicCoreRepairMeta,
...
async function committeeCall(body){
  ...
  if(hasVision&&local.enabled&&local.localOnly){
    try{
      return await runLocalVisionCommittee(body);
    }catch(localError){
      // V109_EXPLICIT_FREE_QUOTA_FAILOVER: automated Leader Auto never burns
      // Kiro quota implicitly. Only a caller that explicitly opts in may use
      // the connected-account free-quota Vision rescue path.
      if(body?.allowFreeQuotaVisionFallback===true){
        return runKiroFreeQuotaVisionFallback(body,localError);
      }
      localError.freeQuotaFallback={attempted:false,reason:'EXPLICIT_OPT_IN_REQUIRED'};
      throw localError;
    }
  }
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='POST'&&u.pathname==='/live/execute'){
      // R2539: external/mobile order origination is permanently disabled.
      // Leader AUTO / fast lane execute inside the PC controller and never need this HTTP endpoint.
      // This server-side barrier protects the account even if an OLD APK remains installed.
      try{await readBody(req);}catch{}
      return send(res,410,{
        ok:false,orderPlaced:false,liveAllowed:false,
        execution:'REMOTE_LIVE_EXECUTE_DISABLED_PC_SCHEDULER_ONLY',
        reasons:['REMOTE_LIVE_EXECUTE_DISABLED_PC_SCHEDULER_ONLY'],
        note:'PC BrainHub scheduler is the sole automated entry executor. Android may only arm/disarm/configure/read telemetry.'
      });
    }
    ...
    if(req.method==='GET'&&u.pathname==='/models/routes'){
      ...
      return send(res,200,{
        ...
        localVisionSingleTfFallback:true,
        visionProfile:v110.visionProfile,
        tradeLanePolicy:v110.tradeLanePolicy,
        ...
        planWorkerRouter:'9ROUTER_FREE_TEXT',
        openRouterFreeWorker:freeWorker.status(),
        note:local.enabled
          ? (local.localOnly
    ...
    if(req.method==='GET'&&u.pathname==='/vision/benchmark'){
      // CLAUDE_V112_VISION_BENCHMARK_21: varsayılan 21 vaka (7 sınıf × 3 boyut). set=v109 eski 3 vaka.
      const setName=u.searchParams.get('set')==='v109'?'v109':'v112';
      const cases=setName==='v109'?syntheticVisionCases():syntheticVisionCasesV112();
      const labels=setName==='v109'?['BOS_UP','SWEEP_RECLAIM','BULL_FVG']:V112_LABELS;
      if(u.searchParams.get('run')!=='1'){
        return send(res,200,{ok:true,execution:'ADVISORY_ONLY',runRequired:true,set:setName,cases:cases.map(x=>x.id),labels,note:'run=1 yerel Vision modelinde sentetik grafik okuma doğruluğunu ölçer (sınıf ve boyut bazında); Kiro/ücretli fallback kullanmaz. Tek GPU: çalışırken Leader AUTO analizi bekler.'});
      }
      const local=localVisionConfig();
      if(!local.enabled||!local.models.length)return send(res,503,{ok:false,error:'local vision unavailable'});
      try{
        const model=local.models[0];
        const results=await withLocalVisionSingleFlight(async()=>{
          const out=[];
          for(const c of cases){
            const size=c.size||{width:896,height:504};
            const png=market.renderChartPng(c.chart,'annotated',{outputWidth:size.width,outputHeight:size.height});
            const dataUrl='data:image/png;base64,'+png.toString('base64');
            const input=multimodalUserContent(
              'SYNTHETIC_VISION_BENCHMARK. Bu gerçek piyasa değildir. Son mum(lar)a bakarak grafiği görsel olarak sınıflandır. '+
              'BOS_UP=son mum aralığın üstünde güçlü kapanış; BOS_DOWN=altında güçlü kapanış; SWEEP_RECLAIM=uzun alt fitil aralığın altına iner, kapanış aralık içinde; '+
              'SWEEP_REJECT=uzun üst fitil aralığın üstüne çıkar, kapanış aralık içinde; BULL_FVG=yukarı boşluklu üç mum; BEAR_FVG=aşağı boşluklu üç mum; RANGE=belirgin olay yok. '+
              'Yalnız bir satır döndür: PATTERN: '+labels.join(' | ')+'. Başka metin yazma.',
              [{tf:c.chart?.frame||'15m',mode:'annotated',dataUrl}]
            );
            const started=Date.now();
            const reply=await callLocalStage('VISION_BENCHMARK='+c.id,model,[
              {role:'system',content:'Synthetic chart-reading benchmark only. Inspect pixels; do not trade and do not infer real market facts.'},
              {role:'user',content:input.content}
            ],local.timeoutMs,{temperature:0,maxTokens:64});
            const actual=parseBenchmarkLabel(reply.text);
            out.push({id:c.id,expected:c.label,actual,match:actual===c.label,profile:c.profile||'main15m',size:size.width+'x'+size.height,durationMs:Date.now()-started});
          }
          return out;
        });
        const summary=summarizeBenchmark(results);
        try{store.journal('VISION_BENCHMARK',null,{set:setName,model,summary,results});}catch{}
        return send(res,200,{ok:true,set:setName,model,...summary,execution:'ADVISORY_ONLY',results});
      }catch(e){
        return send(res,503,{ok:false,error:'vision benchmark failed',detail:String(e?.message||e).slice(0,600),execution:'ADVISORY_ONLY'});
      }
    }
    ...
    if(req.method==='POST'&&u.pathname==='/committee'){
      ...
      if(vision.images.length&&directLocal.enabled&&directLocal.localOnly){
        try{
          const out=await committeeCall(j);
          return send(res,200,out);
        }catch(e){
          const c=e&&e.committee&&typeof e.committee==='object'?e.committee:null;
          return send(res,503,c||{ok:false,error:'no vision analyst replies',detail:String(e?.message||e).slice(0,800),freeQuotaFallback:e?.freeQuotaFallback||null});
        }
      }
      ...
      // PLAN_WORKER_FREE_ONLY_COMMITTEE: worker monitoring is allowed to use
      // 9Router free analysts only. Kiro judges are intentionally unreachable here.
      const needJudge=!freeOnlyCommittee&&(j.forceJudge===true||(ccfg.judgeOnlyOnDisagreement!==false&&disagreement));
...
const planWorkerTimer=setInterval(async()=>{
  try{
    // Text-only plan workers deliberately remain independent from local Vision.
    // They may monitor an existing WATCH condition while the expensive 9TF
    // pipeline is busy on another symbol. A worker can only schedule/escalate;
    // it never qualifies a plan and never reaches the live executor.
    const out=await live.planWorkerTick();
    if(out?.handled===true&&out?.state&&out.state!=='WAIT'){
      log('PLAN WORKER symbol='+(out.symbol||'unknown')+' state='+out.state+' source='+(out.source||'unknown'));
    }else if(out?.ok===false&&out?.skipped!==true){
      log('PLAN WORKER ERROR '+String(out?.reason||'unknown').slice(0,180));
    }
  }catch(e){log('PLAN WORKER TIMER '+String(e?.message||e).slice(0,180));}
},30000);
if(typeof planWorkerTimer.unref==='function')planWorkerTimer.unref();

const activePositionReviewTimer=setInterval(async()=>{
  try{
    if(!backgroundAnalysisWindowOpen())return;
...
const leaderAutoTimer=setInterval(async()=>{
  try{
    // Always enter leaderAutoTick so busy/pipeline skips are counted in OTO health.
    // The controller itself remains single-flight and never overlaps live execution.
...
// CLAUDE_V111_TRAILING_RUNNER: açık runner pozisyonlarında 30 sn'de bir koruyucu stop yönetimi
// (reduce-only; yeni pozisyon açmaz). Açık runner kaydı yoksa Binance'e hiç istek atılmaz.
const claudeRunnerTimer=setInterval(async()=>{
  try{
    const out=await live.runnerTick();
    for(const r of Array.isArray(out?.results)?out.results:[]){
      if(r&&r.action&&!['NONE','HOLD','COOLDOWN'].includes(r.action))log('CLAUDE RUNNER symbol='+(r.symbol||'?')+' action='+r.action+' phase='+(r.phase||'?')+(r.target!=null?' target='+r.target:'')+(r.reason?' reason='+r.reason:''));
    }
  }catch(e){log('CLAUDE RUNNER TIMER '+String(e?.message||e).slice(0,180));}
},30000);
