...
$ExpectedOffice = '2.0.7-JEV-FULL-MIRROR-R2540'
...
$Work = Join-Path $InstallRoot 'R2540-JEV-FULL-MIRROR'
...
Write-Host 'R2540_PRECHECK_LIVE_OFF'
...
if ($null -eq $mirror) {
  if ($null -eq $lastMirror) { throw "R2540 mirror endpoint verification failed: $mirrorLastError" }
  ...
  $mirror = $lastMirror
  Write-Warning "R2540_RUNTIME_PARITY_DIAGNOSTIC $mirrorLastError"
}
...
if (-not ($health.features -contains 'JEV_LIVE_MIRROR_READ_ONLY')) { throw 'R2540 live mirror marker missing.' }
if (-not ($health.features -contains 'JEV_MIRROR_PACKET_CHART_PARITY')) { throw 'R2540 packet/chart parity marker missing.' }
if (-not ($health.features -contains 'JEV_LIVE_MIRROR_FULL_OVERLAYS')) { throw 'R2540 full-overlay mirror marker missing.' }
...
if ([string]$health.jevSovereign.liveMirror.version -ne 'R2.5.4.0') { throw 'R2540 live mirror metadata missing.' }
...
if (-not $mirror.ok -or [string]$mirror.contract -ne 'R2540_JEV_LIVE_MIRROR') { throw 'R2540 JEV live mirror endpoint failed.' }
...
if ($installedOfficeSource -notmatch 'JEV Canlı Görüş Aynası — Tam Görünüm') { throw 'R2540 Office full mirror card missing.' }
if ($installedOfficeSource -notmatch 'Observed liquidation zones') { throw 'R2540 liquidation-zone telemetry missing.' }
if ($installedOfficeSource -notmatch 'FULL ANNOTATED') { throw 'R2540 full annotated graph missing.' }
...
Write-Host 'R2540_PC_UPDATE_OK'
Write-Host 'R2540_OFFICE_UPDATE_OK'
Write-Host 'R2540_CORTEX_RUNTIME_LOADED_OK'
Write-Host 'R2540_MEMORY_LIFETIME_OK'
Write-Host 'R2540_LIFETIME_MEMORY_OK'
Write-Host 'R2540_RESEARCH_DESK_OK'
Write-Host 'R2540_RESEARCH_RETRY_OK'
Write-Host 'R2540_OSS_REFERENCE_OK'
Write-Host 'R2540_VISION_SCHEMA_REPAIR_OK'
Write-Host 'R2540_VISION_EVIDENCE_ONLY_OK'
Write-Host 'R2540_VISION_AUDIT_TOOL_OK'
Write-Host 'R2540_CONTEXT_COMPLETE_OK'
Write-Host 'R2540_JEV_LIVE_MIRROR_OK'
Write-Host 'R2540_REMOTE_EXECUTE_BLOCK_OK'
if ([int]$mirror.parity.mismatches -eq 0) {
  Write-Host 'R2540_PACKET_CHART_PARITY_OK'
} else {
  Write-Host ("R2540_PACKET_CHART_PARITY_VISIBLE mismatches={0}" -f $mirror.parity.mismatches)
}
Write-Host 'R2540_OFFICE_MIRROR_CARD_OK'
Write-Host 'R2540_FULL_OVERLAY_GRAPH_OK'
Write-Host 'R2540_LIQUIDATION_OVERLAY_OK'
Write-Host 'R2540_FULL_WIDTH_LAYOUT_OK'
Write-Host 'R2540_SETUP_TAXONOMY_OK'
Write-Host 'R2540_ENTRY_TIMING_OK'
Write-Host 'R2540_VISION_SMC_OVERLAYS_OK'
Write-Host 'R2540_FREE_RESEARCH_OK'
Write-Host 'R2540_EXIT_BINDING_OK'
Write-Host 'R2540_PARTIAL_BINDING_OK'
Write-Host 'R2540_LIVE_VERIFIED_OFF'
