...
test('R2540 BrainHub exposes read-only full-overlay JEV live mirror with deterministic packet-chart parity',()=>{
  const server=fs.readFileSync(path.join(__dirname,'..','server.js'),'utf8');
  assert.match(server,/\/context\/jev-live-mirror/);
  assert.match(server,/R2540_JEV_LIVE_MIRROR/);
  ...
  assert.match(server,/R2540_FULL_SMC_LIQUIDATION/);
...
test('R2540 Office full mirror remains GET-only and proxies only validated chart/mirror inputs',()=>{
  ...
  assert.match(server,/2\.0\.7-JEV-FULL-MIRROR-R2540/);
  ...
  assert.match(html,/Range HIGH\/LOW\/EQ/);
