...
OpenCode sağlayıcısı ücretsiz katmanı 9Router REST proxy üzerinden `403 FreeTierError` ile reddederse BrainHub yalnız belgelenmiş ücretsiz modeller için OpenCode'un resmi free inference uç noktasını dener. Bu fallback ücretli/Kiro modele geçmez ve kimlik/istemci taklidi yapmaz. 9TF Vision için `mimo-v2.5-free` önceliklidir; gerçek görsel okuma yine `/vision/probe` ile doğrulanır. `/opencode/status` resmi model kataloğuna erişimi ve MiMo/Big Pickle görünürlüğünü gösterir.
