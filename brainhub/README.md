...
OpenCode sağlayıcısı ücretsiz katmanı 9Router üzerinden `403 FreeTierError` ile reddederse BrainHub, kullanıcı ayrıca bir OpenRouter API anahtarı yapılandırdıysa yalnız Vision isteklerinde `openrouter/free` rotasına düşebilir. Bu rota OpenRouter'ın o anda ücretsiz ve görsel girişini destekleyen modellerinden birini seçer; Kiro/ücretli fallback açılmaz. Anahtar `& 'C:\BrainHub\VISION-SETUP.ps1'` ile gizli girilir ve DPAPI ile yerel Windows kullanıcısına bağlı saklanır. `/opencode/status` veya `VISION-STATUS.ps1` yalnız yapılandırma durumunu gösterir; anahtarı asla döndürmez. Gerçek 9TF görsel okuma başarısı yine `TEST.ps1 -Deep` ve `/vision/probe` ile doğrulanır.
