...
function Test-Brain([string]$BrainRoot, [switch]$IncludeDeep) {
    ...
    $sv = Get-PropValue $h 'jevSovereign' $null
    if ($null -eq $sv -or -not [bool](Get-PropValue $sv 'enabled' $false) -or [string](Get-PropValue $sv 'decisionOwner' '') -ne 'JEV' -or
        [int](Get-PropValue $sv 'passLimit' 0) -ne 2 -or [bool](Get-PropValue $sv 'autonomousPlanWorkers' $true) -or
        -not [bool](Get-PropValue $sv 'noScoreThresholds' $false) -or -not [bool](Get-PropValue $sv 'noTwoOfThreeGate' $false) -or
        -not [bool](Get-PropValue $sv 'noHard15mStrategicVeto' $false)) {
        throw 'R2.5.3.2 JEV SOVEREIGN contract dogrulanamadi.'
    }
    $svLanes = @((Get-PropValue $sv 'primaryLanes' @()))
    if (-not ($svLanes -contains '5m') -or -not ($svLanes -contains '15m')) { throw 'R2.5.3.2 5m/15m lane contract eksik.' }
    Write-Host ("R2532_SOVEREIGN owner={0} lanes={1} passLimit={2} workers=ON_DEMAND" -f (Get-PropValue $sv 'decisionOwner' ''),($svLanes -join ','),(Get-PropValue $sv 'passLimit' 0))
    ...
    $jevStatus = Get-PropValue $live 'jev' $null
    if ($null -eq $jevStatus -or [string](Get-PropValue $jevStatus 'mode' '') -ne 'SOVEREIGN_DIRECTOR_5M15M') { throw 'JEV runtime mode SOVEREIGN_DIRECTOR_5M15M degil.' }
...
$files = @('server.js','scanner.js','leader-committee.js','leader-live-intent.js','engine.js','market.js','market-maker-evidence.js','pipeline.js','store.js','risk-gate.js','binance-dry-run-executor.js','binance-account-context.js','live-authorization.js','binance-live-transport.js','live-controller.js','position-manager.js','jev-decision.js','knowledge-research.js','plan-workers.js','openrouter-free-worker.js','wait-condition.js','vision-benchmark.js','claude-v109.js','trade-lanes.js','v110.js','claude-v111.js','claude-v112.js')
