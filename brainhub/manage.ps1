...
function Test-Brain([string]$BrainRoot, [switch]$IncludeDeep) {
    ...
    if ([string]$h.featureVersion -ne '9.5.110-VISION') { throw "Beklenen PC Brain Hub surumu 9.5.110-VISION; gelen=$($h.featureVersion)" }
...
try {
    ...
    foreach ($cfg in @(@('models.example.json','models.json'),@('committee.example.json','committee.json'),@('claude-v109.example.json','claude-v109.json'))) {
