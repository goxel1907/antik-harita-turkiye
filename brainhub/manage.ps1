...
function Get-Source([string]$Given) {
    ...
    if ($serverText -notmatch 'KIRO_FREE_QUOTA_VISION_OPT_IN' -or $serverText -notmatch 'VISION_PIXEL_PROBE' -or $serverText -notmatch 'KKK_DETAILED_9TF_DIAGNOSTICS' -or $serverText -notmatch 'LEADER_DETAIL_PROBE' -or $serverText -notmatch 'VISION_RUNTIME_TRUTH_STATUS' -or $serverText -notmatch 'LEADER_STATUS_STALE_SUPPRESSION' -or $serverText -notmatch 'LEADER_AUTO_HEALTH_TELEMETRY' -or $serverText -notmatch 'LEADER_AUTO_COVERAGE_SCHEDULER' -or $serverText -notmatch 'OPENCODE_OFFICIAL_FREE_INFERENCE') {
