...
function Test-Brain([string]$BrainRoot, [switch]$IncludeDeep) {
    ...
    if ([string](Get-PropValue $routes 'planWorkerRouter' '') -ne '9ROUTER_FREE_TEXT') { throw 'v9.5.107 plan worker 9Router text rotasi eksik.' }
    ...
    if ($null -eq $freeWorkerRoute -or -not [bool](Get-PropValue $freeWorkerRoute 'freeOnly' $false) -or [string](Get-PropValue $freeWorkerRoute 'model' '') -ne 'openrouter/free') {
        throw 'v9.5.107 OpenRouter free-only worker rotasi dogrulanamadi.'
    }
    ...
    if ($null -eq $planWorkerStatus -or -not [bool](Get-PropValue $planWorkerStatus 'enabled' $false) -or -not [bool](Get-PropValue $planWorkerStatus 'parallelWithVision' $false) -or [string](Get-PropValue $planWorkerStatus 'routineRouter' '') -ne '9ROUTER_FREE_TEXT') {
        throw 'v9.5.107 plan worker runtime durumu etkin degil.'
    }
...
if ($Action -eq 'VisionLocalSetup') {
    ...
    try {
        ...
        Test-Brain $rootFull
        $routes = Invoke-RestMethod -Uri 'http://127.0.0.1:8787/models/routes' -Headers (Auth-Headers $rootFull) -TimeoutSec 10
        if (-not $routes.localVisionEnabled -or -not $routes.localVisionOnly -or -not $routes.localVisionTwoStage -or [int]$routes.localVisionBatchSize -ne 1 -or -not $routes.localVisionSingleTf -or -not $routes.localVisionCompactFinalize -or -not $routes.localVisionTfContract -or -not $routes.localVisionSplitGlobal -or -not $routes.localVisionDirectPipeline -or @($routes.localVisionModels) -notcontains ('local/' + $runtimeModel) -or [int]$routes.localVisionContextSize -lt $contextSize) {
...
$files = @('server.js','scanner.js','leader-committee.js','leader-live-intent.js','engine.js','market.js','pipeline.js','store.js','risk-gate.js','binance-dry-run-executor.js','binance-account-context.js','live-authorization.js','binance-live-transport.js','live-controller.js','position-manager.js','jev-decision.js','plan-workers.js','openrouter-free-worker.js')
