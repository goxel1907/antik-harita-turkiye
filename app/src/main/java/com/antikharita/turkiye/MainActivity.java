    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebViewClient(new WebViewClient(){
            @Override public void onPageFinished(WebView view,String url){
                ...
                view.evaluateJavascript("(function(){function add(id,src){if(document.getElementById(id))return;var s=document.createElement('script');s.id=id;s.src=src;document.body.appendChild(s);}add('hotfix-v10','hotfix-v10.js');setTimeout(function(){add('protection-module','protection.js');},100);})()",null);
            }
        });
    ...
    public class Bridge {
        @JavascriptInterface public void fetchOverpassV2(String requestId,String query){
            ...
            new Thread(()->{
                ...
                for(String ep:endpoints){try{payload=postForm(ep,"data="+URLEncoder.encode(query,"UTF-8"),5000,14000);if(payload!=null&&!payload.isEmpty()){source=host(ep);break;}}catch(Exception e){if(errs.length()>0)errs.append(" | ");errs.append(host(ep)).append(": ").append(e.getClass().getSimpleName());}}
        ...
        @JavascriptInterface public void geocode(String text){
            ...
            new Thread(()->{try{String q=URLEncoder.encode(text.trim(),"UTF-8");String out=getText("https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=12&countrycodes=tr&q="+q,6000,12000);runOnUiThread(()->webView.evaluateJavascript("window.onNativeGeocode("+JSONObject.quote(out)+")",null));}catch(Exception e){String m=e.getClass().getSimpleName()+": "+String.valueOf(e.getMessage());runOnUiThread(()->webView.evaluateJavascript("window.onNativeGeocodeError("+JSONObject.quote(m)+")",null));}}).start();
        }
